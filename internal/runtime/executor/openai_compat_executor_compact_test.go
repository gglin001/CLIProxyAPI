...
func TestOpenAICompatExecutorDeepSeekThinkingPreservesEmptyReasoningContent(t *testing.T) {
	var gotBody []byte
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
		gotBody = body
	...
	payload := []byte(`{
		...
		"messages":[
			...
			{"role":"assistant","tool_calls":[{"id":"call_probe","type":"function","function":{"name":"exec_command","arguments":"{}"}}]},
			{"role":"tool","tool_call_id":"call_probe","content":"ok"},
	...
	if !reasoning.Exists() {
		t.Fatalf("messages.1.reasoning_content should exist; body=%s", string(gotBody))
	}
	if reasoning.Type != gjson.String {
		t.Fatalf("messages.1.reasoning_content type = %v, want string; body=%s", reasoning.Type, string(gotBody))
	}
	if got := reasoning.String(); got != "" {
		t.Fatalf("messages.1.reasoning_content = %q, want empty string; body=%s", got, string(gotBody))
	}
}

func TestNormalizeOpenAICompatReasoningToolCalls_ReplacesNonStringReasoningContent(t *testing.T) {
	body := []byte(`{
		"model":"deepseek-v4-pro",
		"reasoning_effort":"high",
		"messages":[
			{"role":"assistant","tool_calls":[{"id":"call_1","type":"function","function":{"name":"read","arguments":"{}"}}],"reasoning_content":null}
		]
	}`)

	out, err := normalizeOpenAICompatReasoningToolCalls(body, "deepseek-v4-pro")
	if err != nil {
		t.Fatalf("normalizeOpenAICompatReasoningToolCalls() error = %v", err)
	}
	reasoning := gjson.GetBytes(out, "messages.0.reasoning_content")
	if !reasoning.Exists() {
		t.Fatalf("messages.0.reasoning_content should exist; body=%s", string(out))
	}
	if reasoning.Type != gjson.String {
		t.Fatalf("messages.0.reasoning_content type = %v, want string; body=%s", reasoning.Type, string(out))
	}
	if got := reasoning.String(); got != "" {
		t.Fatalf("messages.0.reasoning_content = %q, want empty string; body=%s", got, string(out))
	}
}
